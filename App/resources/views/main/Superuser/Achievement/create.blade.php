...
@section('content')
    ...
    <div class="mb-0">
        <div class="card p-8">
            <div class="max-w-xl">
                <x-form :action="route('superuser.achievement.store')">
                    <x-form-group>
                        <x-label for="type">{{ __('Тип') }}</x-label>
                        <x-select id="type" name="type" :option="\App\Enums\Achievement\TypeEnum::asSelectArray()" :o_selected="[old('type')]" :invalid="$errors->has('type')"></x-select>
                    ...
                    <x-form-group>
                        ...
                        <x-input id="purpose" name="purpose" :value="old('purpose')" :invalid="$errors->has('purpose')"></x-input>
                        <x-invalid-feedback :messages="$errors->get('purpose')"></x-invalid-feedback>
                    </x-form-group>

                    <x-form-group>
                        <x-label for="status">{{ __('Статус') }}</x-label>
                        <x-select id="status" name="status" :option="\App\Enums\Achievement\StatusEnum::asSelectArray()" :o_selected="[old('status')]" :invalid="$errors->has('status')"></x-select>
