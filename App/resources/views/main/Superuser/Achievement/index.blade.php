...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex items-center gap-2">
            <x-a-button-primary :href="route('superuser.achievement.create')">{{ __('Новая запись') }}</x-a-button-primary>
        </div>
    </div>
                ...
                <table class="table">
                    <thead class="table__thead">
                        <tr class="table__tr">
                            ...
                            <th class="table__th">{{ __('Текстовое описание') }}</th>
                            <th class="table__th">{{ __('Награда') }}</th>
                            <th class="table__th">{{ __('Цель') }}</th>
                    ...
                    <tbody class="table__tbody">
                        ...
                        @else
                            @foreach($collection as $object)
                                <tr class="table__tr">
                                    <td class="table__td"><x-link :href="route('superuser.achievement.show', $object->id)">{{ $object->id }}</x-link></td>
                                    <td class="table__td">{{ $object->type->description }}</td>
                                    <td class="table__td">{{ str($object->description)->limit(32) }}</td>
                                    <td class="table__td">{{ $object->tether }}</td>
                                    <td class="table__td">{{ $object->purpose }}</td>
