...
@section('content')
    <div class="mb-8">
        <h1 class="h3">{{ $transaction->id }}</h1>
    </div>

    <div class="mb-8">
        <div class="flex items-center gap-2">
            <x-a-button-primary :href="route('superuser.transaction.edit', $transaction->id)">{{ __('Редактировать') }}</x-a-button-primary>

            <x-form :action="route('superuser.transaction.destroy', $transaction->id)">
                @method('DELETE')
    ...
    <div class="mb-0">
        <ul class="card flex flex-col">
            ...
            <li class="flex items-center border-b border-slate-300 p-5 last:border-0 break-all">
                <div class="w-[40%] shrink-0">{{ __('Источник') }}:</div>
                <div>{{ $transaction->source->description }}</div>
            </li>
            <li class="flex items-center border-b border-slate-300 p-5 last:border-0 break-all">
                <div class="w-[40%] shrink-0">{{ __('Tether') }}:</div>
                <div>{{ $transaction->tether }}</div>
            </li>
            <li class="flex items-center border-b border-slate-300 p-5 last:border-0 break-all">
                <div class="w-[40%] shrink-0">{{ __('Ip') }}:</div>
                <div>{{ $transaction->ip?->ip }}</div>
            </li>
            <li class="flex items-center border-b border-slate-300 p-5 last:border-0 break-all">
                <div class="w-[40%] shrink-0">{{ __('User-Agent') }}:</div>
                <div>{{ $transaction->userAgent?->user_agent }}</div>
            </li>
            <li class="flex items-center border-b border-slate-300 p-5 last:border-0 break-all">
