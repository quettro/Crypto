...
@section('content')
    ...
    <div class="mb-0">
        <div class="card p-8">
            <div class="max-w-xl">
                <x-form :action="route('superuser.user.store')">
                    <x-form-group>
                    ...
                    <x-form-group>
                        ...
                        <x-input id="email_verified_at" name="email_verified_at" :value="old('email_verified_at')" :invalid="$errors->has('email_verified_at')"></x-input>
                    ...
                    <x-form-group>
                        ...
                        <x-input id="remember_token" name="remember_token" :value="old('remember_token')" :invalid="$errors->has('remember_token')"></x-input>
                        <x-invalid-feedback :messages="$errors->get('remember_token')"></x-invalid-feedback>
                    </x-form-group>

                    <x-form-group>
                        <x-label for="status">{{ __('Статус') }}</x-label>
                        <x-select id="status" name="status" :option="\App\Enums\User\StatusEnum::asSelectArray()" :o_selected="[old('status')]" :invalid="$errors->has('status')"></x-select>
                        <x-invalid-feedback :messages="$errors->get('status')"></x-invalid-feedback>
                    </x-form-group>

                    <x-form-group>
                        <x-label for="superuser">{{ __('Суперпользователь') }}</x-label>
                        <x-select id="superuser" name="superuser" :option="\App\Enums\User\SuperuserEnum::asSelectArray()" :o_selected="[old('superuser')]" :invalid="$errors->has('superuser')"></x-select>
                        <x-invalid-feedback :messages="$errors->get('superuser')"></x-invalid-feedback>
                    </x-form-group>

                    <x-form-group>
                        <x-label for="balance">{{ __('Баланс') }}</x-label>
                        <x-input id="balance" name="balance" :value="old('balance')" :invalid="$errors->has('balance')"></x-input>
                    ...
                    <x-form-group>
                        ...
                        <x-input id="referer" name="referer" :value="old('referer')" :invalid="$errors->has('referer')"></x-input>
