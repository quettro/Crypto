...
@section('content')
    ...
    <div class="mb-8">
        <div class="flex items-center gap-2">
            <x-a-button-primary :href="route('superuser.link.create')">{{ __('Новая запись') }}</x-a-button-primary>
        </div>
    </div>
                    ...
                    <tbody class="table__tbody">
                        ...
                        @else
                            @foreach($collection as $object)
                                <tr class="table__tr">
                                    <td class="table__td"><x-link :href="route('superuser.link.show', $object->id)">{{ $object->id }}</x-link></td>
