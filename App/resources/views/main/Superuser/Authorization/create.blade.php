...
@section('content')
    ...
    <div class="mb-0">
        <div class="card p-8">
            <div class="max-w-xl">
                <x-form :action="route('superuser.authorization.store')">
                    <x-form-group>
                        <x-label for="code">{{ __('Код') }}</x-label>
                        <x-select id="code" name="code" :option="\App\Enums\Authorization\CodeEnum::asSelectArray()" :o_selected="[old('code')]" :invalid="$errors->has('code')"></x-select>
