...
@section('content')
    ...
    <div class="mb-0">
        <div class="card p-8">
            <div class="max-w-xl">
                <x-form :action="route('superuser.feedback.store')">
                    <x-form-group>
                        <x-label for="topic">{{ __('Тема') }}</x-label>
                        <x-select id="topic" name="topic" :option="\App\Enums\Feedback\TopicEnum::asSelectArray()" :o_selected="[old('topic')]" :invalid="$errors->has('topic')"></x-select>
