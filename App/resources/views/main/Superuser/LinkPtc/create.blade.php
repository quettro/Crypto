...
@section('content')
    ...
    <div class="mb-0">
        <div class="card p-8">
            <div class="max-w-xl">
                <x-form :action="route('superuser.link-ptc.store')">
                    <x-form-group>
                        <x-label for="uniqid">{{ __('Uniqid') }}</x-label>
                        <x-input id="uniqid" name="uniqid" :value="old('uniqid')" :invalid="$errors->has('uniqid')"></x-input>
                        <x-invalid-feedback :messages="$errors->get('uniqid')"></x-invalid-feedback>
                    </x-form-group>

                    <x-form-group>
                        <x-label for="status">{{ __('Статус') }}</x-label>
                        <x-select id="status" name="status" :option="\App\Enums\LinkPtc\StatusEnum::asSelectArray()" :o_selected="[old('status')]" :invalid="$errors->has('status')"></x-select>
                        <x-invalid-feedback :messages="$errors->get('status')"></x-invalid-feedback>
                    </x-form-group>

                    <x-form-group>
                        <x-label for="link_id">{{ __('Ссылка') }}</x-label>
                        <x-select id="link_id" name="link_id" :option="\App\Models\Link::get()->pluck('name', 'id')->toArray()" :o_selected="[old('link_id')]" :invalid="$errors->has('link_id')"></x-select>
